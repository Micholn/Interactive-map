    ...
    <main className="min-h-screen bg-[#eaf9fb] text-[#071315]">
      <div className="mx-auto flex min-h-screen max-w-[1440px] gap-5 p-5">
        <aside className="sticky top-5 hidden h-[calc(100vh-40px)] w-[72px] flex-col items-center rounded-[34px] bg-white/55 py-4 shadow-[0_20px_70px_rgba(14,74,84,.12)] backdrop-blur md:flex">
          <div className="mb-9 grid h-12 w-12 place-items-center rounded-full bg-[#071315] text-[#dfff47]"><Zap size={25} fill="currentColor" /></div>
          <nav className="flex flex-1 flex-col items-center gap-5">
            {[HomeIcon, BrainCircuit, Heart, Activity, Settings].map((Icon, idx) => (
              <button key={idx} className={`grid h-12 w-12 place-items-center rounded-2xl transition ${idx === 1 ? 'bg-[#008393] text-[#dfff47] shadow-[0_18px_34px_rgba(0,131,147,.25)]' : 'text-[#071315]/80 hover:bg-white'}`}>
                <Icon size={20} fill={idx === 2 ? 'currentColor' : 'none'} />
              </button>
            ))}
          </nav>
          <button className="grid h-12 w-12 place-items-center rounded-2xl text-[#071315]/80 hover:bg-white"><ChevronRight size={20} /></button>
        </aside>
        ...
        <section className="flex-1 space-y-5 overflow-hidden">
          <header className="grid gap-4 lg:grid-cols-[1fr_410px]">
            <div className="relative">
              ...
              <p className="mt-5 max-w-xl text-lg font-semibold text-[#496066]">A glamorous dependency dashboard for models, labs, frameworks, compute providers and hardware choke points.</p>
            </div>
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-end gap-3">
                ...
                <button className="grid h-14 w-14 place-items-center rounded-full bg-white/65 shadow-[0_18px_40px_rgba(13,64,74,.12)]"><Gauge size={20} /></button>
                <button className="grid h-14 w-14 place-items-center rounded-full bg-white/65 shadow-[0_18px_40px_rgba(13,64,74,.12)]"><Bell size={20} /></button>
              </div>
              <div className="grid grid-cols-7 gap-2">
                {week.map((d, i) => (
                  <button key={d} className={`rounded-full py-3 text-center text-sm font-black shadow-sm ${i === 1 ? 'bg-[#dfff47]' : 'bg-white/55 text-[#071315]/80'}`}>{d}</button>
                ))}
              </div>
            </div>
          </header>
          ...
          <div className="grid gap-5 xl:grid-cols-[1.18fr_.82fr]">
            <section className="relative overflow-hidden rounded-[32px] bg-white/70 p-8 shadow-[0_25px_80px_rgba(18,73,84,.14)] backdrop-blur">
              <div className="absolute right-5 top-5 grid h-16 w-16 place-items-center rounded-full bg-[#008393] text-[#dfff47] shadow-[0_22px_50px_rgba(0,131,147,.28)]"><Network size={30} /></div>
                ...
                <div className="relative overflow-hidden rounded-[34px] bg-gradient-to-b from-[#f6fbfc] to-[#d7e6e8] p-4">
                  ...
                  <svg className="absolute left-[4%] top-[8%] h-[330px] w-[360px]" viewBox="0 0 360 330" fill="none">
                    {topNodes.map((node, idx) => {
                      ...
                      return <circle key={node.id} cx={pts[0]} cy={pts[1]} r={node.centrality / 10} className="fill-[#007c91] opacity-90" />;
                    })}
                    <path d="M62 115 L130 42 L246 72 L295 171 L204 257 L87 237 Z M62 115 L204 257 M130 42 L295 171 M246 72 L87 237" stroke="#007c91" strokeWidth="4" strokeLinecap="round" opacity=".55" />
                  </svg>
                ...
                <div className="relative hidden pt-10 md:block">
                  ...
                  <div className="mx-auto mt-6 h-[330px] w-[52px] rounded-full bg-white/80 p-2 shadow-inner">
                    <div className="relative h-full rounded-full bg-gradient-to-t from-[#d8e7e9] to-white">
                      <div className="absolute bottom-0 w-full rounded-full bg-[#dfff47]" style={{ height: `${dependencyScore}%` }} />
            ...
            <section className="space-y-5">
              <div className="rounded-[32px] bg-white/72 p-7 shadow-[0_25px_80px_rgba(18,73,84,.14)] backdrop-blur">
                ...
                <div className="mt-5 grid grid-cols-3 items-end gap-4">
                  {(['model','framework','hardware'] as EntityCategory[]).map((cat, i) => {
                    ...
                    return (
                      <button key={cat} onClick={() => setActive(cat)} className={`group rounded-[50%] p-5 transition ${i === 1 ? 'scale-110 bg-[#9eeeff]' : 'bg-[#dffbc2]'} ${active === cat ? 'ring-4 ring-[#dfff47]' : ''}`}>
                ...
                <div className="mt-5 flex items-end justify-between gap-1 px-2">
                  {[60,70,80,90,100,110,120].map((n, i) => <span key={n} className="text-sm font-black text-[#071315]/80">{i === 3 ? <span className="rounded-full bg-[#dfff47] px-3 py-2 text-lg">{n}</span> : n}</span>)}
                </div>
                <div className="mt-4 flex items-center justify-between px-3">
                  {Array.from({ length: 13 }).map((_, i) => <span key={i} className={`h-5 w-[3px] rounded-full ${i === 6 ? 'bg-[#071315]' : 'bg-[#c5dfe3]'}`} />)}
                </div>
              </div>
              ...
              <div className="rounded-[32px] bg-[#007c91] p-7 text-white shadow-[0_25px_80px_rgba(0,93,109,.28)]">
                <div className="flex items-center justify-between">
                  ...
                  <button className="grid h-14 w-14 place-items-center rounded-full bg-[#006b7a]"><Menu size={22} /></button>
                </div>
                <div className="mt-6 grid grid-cols-[1fr_112px] gap-5">
                  ...
                  <div className="grid grid-cols-3 gap-2 self-end">
                    {Array.from({ length: 12 }).map((_, i) => <div key={i} className={`grid h-8 w-8 place-items-center rounded-md ${i < 2 ? 'bg-[#dfff47]' : 'bg-white/75'}`}><Database size={17} className="text-[#007c91]" /></div>)}
                  </div>
                </div>
              </div>
            </section>
          ...
          <section className="grid gap-5 xl:grid-cols-[360px_1fr]">
            <div className="rounded-[32px] bg-white/72 p-7 shadow-[0_25px_80px_rgba(18,73,84,.12)]">
              <div className="flex items-center justify-between"><h2 className="text-2xl font-black">NODE FILTERS</h2><Search size={22} /></div>
              <div className="mt-5 grid grid-cols-2 gap-3">
                {(Object.keys(categoryMeta) as Array<EntityCategory | 'all'>).map((cat) => {
                  const Icon = categoryMeta[cat].icon;
                  return <button key={cat} onClick={() => setActive(cat)} className={`flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-black transition ${active === cat ? categoryMeta[cat].color : 'bg-[#edf7f8]'}`}><Icon size={17} />{categoryMeta[cat].label}</button>;
                })}
              </div>
              <div className="mt-7 rounded-[26px] bg-[#eff9fa] p-5">
                ...
                <div className="mt-4 h-3 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-[#dfff47]" style={{ width: `${selected.centrality}%` }} /></div>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {visibleNodes.slice(0, 12).map((node, idx) => {
                ...
                return (
                  <button key={node.id} onClick={() => setSelected(node)} className={`group min-h-[170px] rounded-[30px] p-5 text-left shadow-[0_20px_55px_rgba(18,73,84,.10)] transition hover:-translate-y-1 ${selected.id === node.id ? 'bg-[#dfff47]' : idx % 4 === 0 ? 'bg-[#007c91] text-white' : 'bg-white/72'}`}>
                    <div className="flex items-start justify-between">
                      <span className={`grid h-12 w-12 place-items-center rounded-2xl ${selected.id === node.id ? 'bg-black text-[#dfff47]' : 'bg-[#eaf9fb] text-[#007c91]'}`}><Icon size={22} /></span>
...
function MiniCard({ icon, value, label, glow = false }: { icon: ReactNode; value: string; label: string; glow?: boolean }) {
  return (
    <div className={`rounded-[26px] p-4 shadow-[0_18px_35px_rgba(7,19,21,.10)] ${glow ? 'bg-[#dfff47]' : 'bg-white/85'}`}>
